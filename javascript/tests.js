$(document).ready(function (){
  ...
  test("PottyMouth basic tests", 2, function () {
    equal(true, true, "Testing is working");
  ...
  test("PottyMouth replacer tests", 2, function () {
    equal(p.pre_replace('Foo "and" bar and ``baz\'\' or not.'), 'Foo &#8220;and&#8221; bar and &#8220;baz&#8221; or not.');
    equal(p.pre_replace('9\'13"and some arcseconds.'), '9&#8217;13&#34;and some arcseconds.');
  });

  test("PottyMouth tokenizer tests", 10, function () {
    var tstr = p.tokenize('A *BOLD* thing');
    equal(tstr[0].content, 'A ');
    equal(tstr[1].content, '*');
    equal(tstr[2].content, 'BOLD');
    equal(tstr[3].content, '* ');
    equal(tstr[4].content, 'thing');
    equal(tstr[0].name, 'TEXT');
    equal(tstr[1].name, 'STAR');
    equal(tstr[2].name, 'TEXT');
    equal(tstr[3].name, 'ITEMSTAR');
    equal(tstr[4].name, 'TEXT');
  });
